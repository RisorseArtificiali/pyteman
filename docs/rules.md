...
| `barrier` | `barrier` required; `role`, `timeout_s` optional | `role` is `wait` (the default) or `open`. `timeout_s` is a positive finite number up to `9223372036.0`, again `threading.TIMEOUT_MAX`, and defaults to 30. It is refused on `role: open`, whatever the number: the open branch does not wait at all, so a limit written there is not a longer wait and not an unlimited one, it is simply ignored. The message names the placement rather than the range, since no value would have made the rule valid. A failed wait is reported and not fatal by default; see "Strict barrier mode" in `docs/firing.md`. |
